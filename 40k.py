...
class KeyColors:
    RESET = '\033[0m'
    BOLD = '\033[1m'
    GREEN_MAIN = '\033[38;5;46m'
    GREEN_DARK = '\033[38;5;28m'
    GREEN_LIGHT = '\033[38;5;120m'
    GREEN_PALE = '\033[38;5;157m'
    WHITE = '\033[38;5;46m'
    ERROR = '\033[38;5;46m'
    WARNING = '\033[38;5;46m'
...
def encrypt_data_key(data):
    return base64.b64encode(data.encode()).decode()

def decrypt_data_key(encrypted_data):
    try:
        return base64.b64decode(encrypted_data.encode()).decode()
    except:
        return "{}"

def luu_thong_tin_ip_key(ip, key, expiration_date):
    data = {ip: {'key': key, 'expiration_date': expiration_date.isoformat()}}
    encrypted_data = encrypt_data_key(json.dumps(data))
    try:
        with open('ltool_key.json', 'w') as file:
            file.write(encrypted_data)
    except Exception as e:
        print(f"{KC.GREEN_MAIN}Lỗi lưu file key: {e}{KC.RESET}")

def tai_thong_tin_ip_key():
    try:
        with open('ltool_key.json', 'r') as file:
            encrypted_data = file.read()
        data = json.loads(decrypt_data_key(encrypted_data))
        return data
    except Exception:
        return None

def kiem_tra_ip_key(ip):
    data = tai_thong_tin_ip_key()
    if not data or ip not in data:
        return None
    info = data[ip]
    if 'expiration_date' not in info:
        try: os.remove('ltool_key.json')
        except: pass
        return None
    try:
...
def generate_key_and_url_key(ip_address):
    ngay = int(datetime.now().day)
    key1 = str(ngay * 27 + 27)
    ip_numbers = ''.join(filter(str.isdigit, ip_address))
    key = f'Mano-{key1}{ip_numbers}'
    expiration_date = datetime.now().replace(hour=23, minute=59, second=59, microsecond=0)
    url = f'https://manokey.neocities.org//api_key/?key={key}'
    return url, key, expiration_date

def da_qua_gio_moi_key():
    now = datetime.now()
    midnight = now.replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(days=1)
    return now >= midnight

def get_shortened_link_phu_key(url):
    try:
        token = "68d632590e2eca0b60503748"
        api_url = f"https://link4m.co/api-shorten/v2?api={token}&url={url}"
        response = requests.get(api_url, timeout=5)
        response.raise_for_status()
        return response.json()
    except Exception as e:
        return {"status": "error", "message": f"Lỗi kết nối dịch vụ rút gọn: {e}"}

def show_ltool_banner():
    os.system("cls" if os.name == "nt" else "clear")
    banner_art = f"""
{KC.GREEN_MAIN}██╗      ████████╗ ██████╗  ██████╗ ██╗     {KC.RESET}
{KC.GREEN_MAIN}██║      ╚══██╔══╝██╔═══██╗██╔═══██╗██║     {KC.RESET}
{KC.GREEN_MAIN}██║         ██║   ██║   ██║██║   ██║██║     {KC.RESET}
{KC.GREEN_MAIN}██║         ██║   ██║   ██║██║   ██║██║     {KC.RESET}
{KC.GREEN_MAIN}███████╗    ██║   ╚██████╔╝╚██████╔╝███████╗{KC.RESET}
...
"""
    print(banner_art)
    print(f"{KC.GREEN_MAIN}{'='*60}{KC.RESET}")
    print(f"{KC.GREEN_MAIN}>> Đơn 40k trả tiền để mở source <<{KC.RESET}")
    print(f"{KC.GREEN_MAIN}{'='*60}{KC.RESET}")
    time.sleep(1)
...
def verify_license_system():
    ...
    print(f"\n{KC.GREEN_MAIN}>> Đang kiểm tra địa chỉ IP...{KC.RESET}")
    ip_address = get_ip_address_key()

    if not ip_address:
        print(f"{KC.GREEN_MAIN}❌ LỖI NGHIÊM TRỌNG: KHÔNG THỂ LẤY IP. TẮT TOOL ĐỂ BẢO MẬT.{KC.RESET}")
        time.sleep(3)
        sys.exit(1)

    print(f"{KC.GREEN_MAIN}IP Address: {KC.GREEN_MAIN}{ip_address}{KC.RESET}")
    print(f"{KC.GREEN_MAIN}{'-'*60}{KC.RESET}")

    existing_key = kiem_tra_ip_key(ip_address)

    if existing_key:
        print(f"\n{KC.GREEN_MAIN}✅ KEY HỢP LỆ! ĐANG KHỞI ĐỘNG L-TOOL...{KC.RESET}")
        time.sleep(2)
        return

    else:
        if da_qua_gio_moi_key():
             print(f"{KC.GREEN_MAIN}KEY ĐÃ HẾT HẠN. VUI LÒNG LẤY LẠI KEY MỚI NHẤT.{KC.RESET}")
        else:
             print(f"{KC.GREEN_MAIN}⚠️ CHƯA CÓ KEY. BẮT ĐẦU QUÁ TRÌNH LẤY KEY FREE.{KC.RESET}")

        url, key, expiration_date = generate_key_and_url_key(ip_address)

        with ThreadPoolExecutor(max_workers=2) as executor:
            print(f"\n{KC.GREEN_MAIN}[1] Lấy Key Free (Vượt link Quảng Cáo) {KC.RESET}")

            while True:
                try:
                    choice = input(f"{KC.GREEN_MAIN}Lựa chọn: {KC.RESET}").strip()

                    if choice == "1":
                        print(f"{KC.GREEN_MAIN}Đang tạo link vượt key...{KC.RESET}")
                        future = executor.submit(get_shortened_link_phu_key, url)
                        data_link = future.result()

                        if data_link and data_link.get('status') == "error":
                            print(f"{KC.GREEN_MAIN}❌ Lỗi: {data_link.get('message')}. Vui lòng thử lại sau.{KC.RESET}")
                            sys.exit(1)
                        else:
                            short_link = data_link.get('shortenedUrl')
                            print(f"\n{KC.GREEN_MAIN}------------------------------------------------------------{KC.RESET}")
                            print(f"{KC.GREEN_MAIN}   ▶️ Vui lòng truy cập link sau để lấy KEY FREE (Còn hiệu lực đến {expiration_date.strftime('%H:%M %d/%m')}) {KC.RESET}")
                            print(f"{KC.GREEN_MAIN}   🔗 Link Key: {short_link}{KC.RESET}")
                            print(f"{KC.GREEN_MAIN}------------------------------------------------------------{KC.RESET}\n")

                        while True:
                            keynhap = input(f"{KC.GREEN_MAIN}🔑 Nhập key đã vượt: {KC.RESET}").strip()
                            if keynhap == key:
                                print(f"\n{KC.GREEN_MAIN}✅ KEY CHÍNH XÁC! KHỞI ĐỘNG L-TOOL...{KC.RESET}")
                                sleep(1.5)
                                luu_thong_tin_ip_key(ip_address, keynhap, expiration_date)
                                return
                            else:
                                print(f"{KC.GREEN_MAIN}❌ Key sai! Vui lòng nhập lại hoặc lấy lại key.{KC.RESET}")
                    else:
                        print(f"{KC.GREEN_MAIN}Vui lòng nhập số 1 để lấy Key Free.{KC.RESET}")

                except KeyboardInterrupt:
                    print(f"\n{KC.GREEN_MAIN}🛠️ Hủy thao tác bởi người dùng.{KC.RESET}")
                    sys.exit()
                except Exception as e:
                    print(f"{KC.GREEN_MAIN}Lỗi hệ thống key: {e}{KC.RESET}")
                    sys.exit(1)

console = Console()

def show_banner_rich():
    console.print(Rule("[bold green]KH TOOL[/]", style="green"))
    console.print("[green]Đơn 40k by [bold]Mano | Đơn 40k [bold green]Sang Phùng[/][/]")
...
RAINBOW_COLORS = ["green", "green1", "spring_green1", "spring_green2", "chartreuse1", "chartreuse2"]
...
def safe_input(prompt: str, default=None, cast=None):
    full_prompt = f"{KC.GREEN_MAIN}{prompt}"
    try:
        s = input(full_prompt).strip()
        print(KC.RESET, end="")
    except EOFError:
        print(KC.RESET, end="")
        return default
...
def place_bet_async(issue: int, room_id: int, amount: float, algo_used: Optional[str] = None):
    def worker():
        console.print(f"[spring_green1]Đang đặt {amount} BUILD -> PHÒNG_{room_id} (v{issue}) — Thuật toán: {algo_used}[/]")
        ...
        if isinstance(res, dict) and (res.get("msg") == "ok" or res.get("code") == 0 or res.get("status") in ("ok", 1)):
            ...
            console.print(f"[bold green]✅ Đặt thành công {amount} BUILD vào PHÒNG_{room_id} (v{issue}).[/]")
        else:
            console.print(f"[bold green]❌ Đặt lỗi v{issue}: {res}[/]")
...
def lock_prediction_if_needed(force: bool = False):
    ...
    if _skip_rounds_remaining > 0:
        if _skip_active_issue != issue_id:
            console.print(f"[spring_green1]⏸️ Đang nghỉ {_skip_rounds_remaining} ván theo cấu hình sau khi thua.[/]")
    ...
    if run_mode == "AUTO" and not skip_next_round_flag:
        ...
        if bld is None:
            console.print("[spring_green1]⚠️ Không lấy được số dư trước khi đặt — bỏ qua đặt ván này.[/]")
        ...
        if current_bet is None:
            ...
            console.print(f"[spring_green1]⚠️ current_bet is None, reset to base_bet: {current_bet}[/spring_green1]")

        amt = float(current_bet)
        if amt <= 0:
            console.print("[spring_green1]⚠️ Số tiền đặt không hợp lệ (<=0). Bỏ qua.[/]")
            prediction_locked = False
            return
        ...
        console.print("[spring_green1]⏸️ TẠM DỪNG THEO DÕI SÁT THỦ[/]")
...
def on_open(ws):
    _ws["ws"] = ws
    console.print("[bold green]ĐANG TRUY CẬP DỮ LIỆU GAME[/]")
    ...
    try:
        ...
        if placed_room != int(krid):
            ...
            try:
                ...
                console.print(f"[bold green]🔴 THUA! Số mới cho ván kế tiếp: {current_bet} BUILD[/bold green]")
            except Exception as e:
                current_bet = base_bet
                console.print(f"[bold green]🔴 THUA! Lỗi tính toán: {e}, reset về: {current_bet} BUILD[/bold green]")
            lose_streak += 1
            win_streak = 0
            ...
            def _check_stop_conditions():
                ...
                try:
                    ...
                    if stop_when_loss_reached and stop_loss_target is not None and isinstance(current_build, (int, float)) and current_build <= stop_loss_target:
                        console.print(f"[bold green]⚠️ STOP-LOSS TRIGGED: {current_build} <= {stop_loss_target}. Dừng tool.[/]")
                        stop_flag = True
                        try:
                            wsobj = _ws.get("ws")
                            if wsobj:
...
def build_header(border_color: Optional[str] = None):
    ...
    left = Text("VUA THOÁT HIỂM VIP", style="bold spring_green1")
    ...
    pnl_style = "bold green" if pnl_val > 0 else ("bold green" if pnl_val < 0 else "green")

    bal = Text.assemble((f"USDT: {u}", "bold spring_green1"), ("   "), (f"XWORLD: {x}", "bold spring_green1"), ("   "), (f"BUILD: {b}", "bold spring_green1"))
    ...
    if stop_when_loss_reached and stop_loss_target is not None:
        right_lines.append(f"[green]StopLoss@{stop_loss_target}[/]")

    right = Text.from_markup("\n".join(right_lines))

    tbl.add_row(left, right)
    tbl.add_row(bal, Text(f"{datetime.now(tz).strftime('%H:%M:%S')}  •  {_spinner_char()}", style="dim green"))
...
def build_rooms_table(border_color: Optional[str] = None):
    t = Table(box=box.MINIMAL, expand=True, style="green")
    ...
    for r in ROOM_ORDER:
        ...
        try:
            if killed_room is not None and int(r) == int(killed_room):
                status = "[bold green]☠ Kill[/]"
        except Exception:
            pass
        try:
            if predicted_room is not None and int(r) == int(predicted_room):
                status = (status + " [dim green]|[/] [green]✓ Dự đoán[/]") if status else "[green]✓ Dự đoán[/]"
        except Exception:
            pass
...
def build_bet_table(border_color: Optional[str] = None):
    t = Table(title="Lịch sử cược (5 ván gần nhất)", box=box.SIMPLE, expand=True, style="green")
...
def prompt_settings():
    ...
    console.print(Rule("[bold spring_green1]CẤU HÌNH NHANH[/]", style="green"))
    ...
    console.print("\n[bold spring_green1]Chọn thuật toán:[/]", style="green")
    console.print("1) VIP50 — 50 công thức tính phòng an toàn nhất (AI + RANDOM + toán học)", style="green")
    console.print("2) VIP50+ — VIP50 mở rộng thêm bias hot/cold", style="green")
    console.print("3) VIP100 — 100 công thức (ensemble lớn)", style="green")
    console.print("4) ADAPTIVE — Bắt đầu như VIP50+ và tự điều chỉnh weights khi có kết quả", style="green")
    console.print("5) VIP5000 — 5000 công thức", style="green")
    console.print("6) VIP5000PLUS — 5000 công thức + lọc AI", style="green")
    console.print("7) VIP10000 — 10000 công thức", style="green")
...
def parse_login():
    ...
    console.print(Rule("[bold spring_green1]ĐĂNG NHẬP[/]", style="green"))
    link = safe_input("Dán link trò chơi (từ xworld.info) tại đây (ví dụ chứa userId & secretKey) > ", default=None)
    if not link:
        console.print("[spring_green1]Không nhập link. Thoát.[/]")
        sys.exit(1)
    try:
        ...
        console.print("[spring_green1]Link không hợp lệ. Thoát.[/]")
...
def main_game():
    ...
    def on_error(msg):
        console.print(f"[green]Balance poll lỗi: {msg}[/]")
    ...
    with Live(Group(build_header(), build_mid(), build_rooms_table(), build_bet_table()), refresh_per_second=8, console=console, screen=False) as live:
        try:
            while not stop_flag:
                ...
                time.sleep(0.12)
            console.print("[bold green]Tool đã dừng theo yêu cầu hoặc đạt mục tiêu.[/]")
        except KeyboardInterrupt:
            console.print("[green]Thoát bằng người dùng.[/]")
            poller.stop()
